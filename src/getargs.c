 ...
 * @implemented
 */
void getargs(char* cmdln, int* argc, char*** argv, char*** env, int expand_wildcards, int* new_mode)
{
   ...
   while (TRUE)
   {
      ...
      if (i >= len || ((cmdln[i] == ' ' || cmdln[i] == '\t') && !inQuotes))
      {
      ...
      if (cmdln[i] == '\"')
      {
         ...
         else if (!inQuotes && i > 0 && cmdln[i - 1] == '\"' && !escapedQuote)
         {
            buffer[bufferIndex++] = '\"';
            ++i;
            escapedQuote = TRUE;
                    ...
                    int expand_wildcards, int* new_mode)
{
   ...
   {
      // Arguments are delimited by white space, which is either a space or a tab.
      if (i >= len || ((wcmdln[i] == ' ' || wcmdln[i] == '\t') && !inQuotes))
      {
         // Handle the case when empty spaces are in the end of the cmdline
         if (anyLetter)
         {
      ...
      if (wcmdln[i] == '\"')
      {
         ...
         else if (!inQuotes && i > 0 && wcmdln[i - 1] == '\"' && !escapedQuote)
         {
            buffer[bufferIndex++] = '\"';
            ++i;
            escapedQuote = TRUE;
